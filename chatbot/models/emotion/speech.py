...
from .base import (
    EMOTION_LABELS,
    handle_emotion_prediction_error,
...
MODEL_PATH_DEFAULT = (
    Path(__file__).resolve().parents[2] / "models" / "cnn_transf_parallel_model.pt"
)
...
# Aligned 7-class labels matching text2emotion output (imported from base)
# EMOTION_LABELS is now imported from .base
...
def load_emotion_model() -> None:
    ...
    resolved_path = (
        Path(ENV_MODEL_PATH).expanduser() if ENV_MODEL_PATH else MODEL_PATH_DEFAULT
    )
    ...
    if torch.cuda.is_available():
        ...
        logger.info("TF32 optimization enabled for speech emotion model")
...
def predict_emotion(audio_path: str, return_logits: bool = False) -> Dict[str, float]:
    ...
    try:
        ...
        model_logits = {
            MODEL_EMOTIONS[i]: float(logits_np[i]) for i in range(len(MODEL_EMOTIONS))
        }
        model_probs = {
            MODEL_EMOTIONS[i]: float(probs_np[i]) for i in range(len(MODEL_EMOTIONS))
        }

        if return_logits:
            # For logits: merge by addition (log-space addition = multiplication in prob-space)
            # This is technically adding log-probabilities
            merged_neutral_logit = np.logaddexp(
                model_logits["neutral"], model_logits["calm"]
            )
        ...
        return handle_emotion_prediction_error(exc, "Speech emotion", return_logits)
